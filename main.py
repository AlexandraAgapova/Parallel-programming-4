...
logging.basicConfig(
    filename=os.path.join(LOG_DIR, "program.log"),
    level=logging.ERROR,
...
class SensorX(Sensor):
    def __init__(
        self, delay: float, data_queue: queue.Queue, stop_event: threading.Event
    ):
        super().__init__()
        self._delay = delay
        self._data = 0
    ...
    def get(self):
        return (
            self.data_queue.get_nowait() if not self.data_queue.empty() else self._data
        )
